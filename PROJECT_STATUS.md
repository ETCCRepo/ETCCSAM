...
**Last updated:** 2026-09-16 — **checkpoint v6.15**: extended password-reset links from 1 hour to 24 hours (explicit request), and fixed a real, previously-unnoticed bug — the member Import History log (`sam_members_import_history`, added in v6.9) was **never actually persisted server-side at all**, silently rejected by `api.php`'s key whitelist since the feature shipped. **⚠️ Bigger change: the checkpoint workflow itself changed this session** — automated regression-test running, long assumed permanently blocked, now works via the Browser pane against the live `test.html` URL (a different mechanism than the old blocked `node run-tests.js` script). The user explicitly asked for this to run automatically from now on, with no manual confirmation step. See "Checkpoint procedure" below — **read it before assuming the old "always ask the user to run tests" rule still applies.**
...
- **Deployed version:** **v6.15** (`index.html` footer `#app-version`) — deployed code matches the latest checkpoint commit, no drift.
- **Git:** `main` branch, last commit `7befaa6` ("Checkpoint v6.15: 24h password reset links, fix Import History persistence"), pushed to `origin`. Working tree is clean. (A small earlier commit this session, `fd77565`, retroactively documented the v6.14 checkpoint that a prior session had left undocumented — see that session's own write-up below.)
- **`test.html` was updated this session and confirmed green by Claude itself**, automatically, via the Browser pane against the live URL — **not** a user-run confirmation, which is the new standing procedure as of this session. All 1228 tests passed.
- **No uncommitted app-code work** as of this doc.
- **⚠️ The checkpoint procedure changed this session — read the "Checkpoint procedure" section below before running a future checkpoint on autopilot from stale memory.** Tests are now run automatically by Claude via the Browser pane; the user no longer needs to run them manually or say "passed."
