...
**Last updated:** 2026-09-14 (later same day) — **checkpoint v6.10**: **Developer-password gate fixed to verify server-side** instead of trusting the browser's local settings copy, root-caused **and fixed the actual password-corruption bug** that open item #1 below had never conclusively closed, added a **separate "Forgot password?" reset flow for the Developer password**, **moved Import Members and Backups from their own Developer screens into collapsible cards inside Settings**, and (found while verifying that reorg) **fixed a real bug where Settings cards with an inline `display:` style never actually collapsed**. This closes out the drift flagged earlier today — everything below is now committed and the deployed code matches the commit.
...
- **Deployed version:** **v6.10** (`index.html` footer `#app-version`) — deployed code matches the latest checkpoint commit, no drift. (Earlier today this doc flagged the opposite — deployed-but-uncommitted work — that gap is now closed.)
- **Git:** `main` branch, last commit `<pending — see checkpoint commit below>` ("Checkpoint v6.10: server-side Developer password verification, reset flow, Settings reorg"), pushed to `origin`. Working tree is clean.
- **`test.html` updated and confirmed green** for this session's full scope: the password-gate server-verification fix, the new `set_password`/`scope`-aware reset flow, the Import Members/Backups move into Settings cards, and the collapsible-card `!important` fix below.
- **New in this session:** the Developer password prompt (`#auth-modal`, `submitAuthPassword()`) now calls a new server action (`verify_settings_password`) instead of comparing against the browser's local copy of `settingsPassword` — this was a **real, previously-unconfirmed bug**, not a stale assumption; see item #1 below, now resolved. Password changes (both Login and Developer/Settings password) go through a new dedicated `set_password` API action instead of the general `save_settings` full-blob save. `forgot_password`/`reset_password` now take a `scope` parameter (`'login'` or `'settings'`) so the Developer password has its own emailed reset link, independent of the login-password reset. Import Members and Backups are no longer separate Developer nav screens — both are now collapsible `.card`s inside the Settings screen itself.
- **⚠️ Real bug found and fixed while verifying the Settings reorg: collapsible cards with their own inline `display:` style never actually collapsed.** The generic `initCollapsibleCards()` mechanism (toggling a `.collapsed` class) was working correctly, but the CSS rule that's supposed to hide a collapsed card's content (`.card.collapsible.collapsed > :not(.card-header) { display: none; }`) was silently losing to any card whose content wrapper set its own inline `display` — an inline style always outranks a class selector regardless of specificity chaining. The "Developer Tools" card hit this (`style="...display:flex;..."` on its body). Fixed by adding `!important` to the collapse rule, which protects every current and future Settings card from the same silent-failure class of bug, not just Developer Tools.
- **⚠️ `deploy.ps1` deploy mechanism changed in the v6.7/v6.8 session — read before assuming curl-based troubleshooting advice from older sessions still applies.** It now uses `System.Net.FtpWebRequest` instead of `curl.exe`. See open item #14 below. (This session hit its known transient `450 File unavailable (file busy)` twice, on `index.html` — both times succeeded on an immediate single retry, consistent with past notes; not a new problem.)
