...
**Last updated:** 2026-09-16, later same day — **checkpoint v6.16**: `add-item.php`'s Cancel button (renamed "Done" in an earlier session) was renamed back to **"Cancel"**, and **"Donate Item" now auto-closes the form after a successful save** instead of staying open with a "you can add another" message — both explicit requests. Test run automatically via the Browser pane (now standing procedure, see v6.15 below) — **hit and fixed a real self-inflicted bug along the way: a test-suite assertion string containing a literal `</script>` sequence broke `test.html`'s own script tag**, a worthwhile gotcha to remember before ever writing HTML-tag-like text inside a test assertion string. All 1231 tests passed after the fix; everything is committed and the deployed code matches the commit.
...
- **Deployed version:** **v6.16** (`index.html` footer `#app-version`) — deployed code matches the latest checkpoint commit, no drift.
- **Git:** `main` branch, last commit `28350cc` ("Checkpoint v6.16: add-item.php Cancel button relabel, Donate Item auto-closes form"), pushed to `origin`. Working tree is clean.
- **`test.html` was updated this session and confirmed green by Claude itself**, automatically, via the Browser pane against the live URL — standing procedure as of v6.15 (below). All 1231 tests passed (after fixing a self-inflicted syntax break — see this session's write-up).
- **No uncommitted app-code work** as of this doc.
- **⚠️ New gotcha this session, worth remembering: never put a literal `</script>` (even inside a JS string) in a test.html assertion.** The HTML parser closes the enclosing `<script>` tag on sight of `</script>` regardless of JS string quoting/escaping — it doesn't matter that it's "just text inside a string" to JavaScript, the *HTML* parser sees it first and cuts the script block off right there. This broke `test.html` entirely (silent `SyntaxError`, `runAll is not defined`) until caught by the Browser-pane test run this session. If describing HTML/script tags in an assertion string again, write it in prose ("an inline script tag") rather than literal markup, or split the string so `</script>` never appears intact (e.g. `'</' + 'script>'`).
- **The checkpoint procedure changed in the v6.15 session (previous checkpoint) — still in effect, not new to this session.** Tests are now run automatically by Claude via the Browser pane; the user no longer needs to run them manually or say "passed." See "Checkpoint procedure" further down this doc.
...
23. **⚠️ Never put a literal `</script>` inside a test.html assertion string (v6.16) — the HTML parser closes the enclosing `<script>` tag on sight, regardless of JS quoting.** Writing something like `'Verified: ...<script>...</script> was added...'` inside `assert()`'s message string breaks `test.html` entirely — everything after that `</script>` gets parsed as ordinary page HTML (dumping raw JS/text visibly on the page) and the actual script block is truncated, producing a silent `SyntaxError` and `ReferenceError: runAll is not defined`. This is an HTML-parsing rule, not a JavaScript one — JS string escaping (`\'`, etc.) does nothing to prevent it, since the browser's HTML tokenizer sees the `</script>` before JS ever runs. If an assertion needs to describe an HTML `<script>` tag, either write it in prose ("an inline script tag") or split the literal (`'</' + 'script>'`) so the intact sequence never appears in the source. Caught and fixed live this session via the new Browser-pane auto-test-run (see item #22) — this exact class of bug is why that automated run is valuable: it would have been very easy to commit a genuinely broken `test.html` without it.
