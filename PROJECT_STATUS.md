...
**Last updated:** 2026-09-14 (later same day, fourth update today) — **checkpoint v6.12**: **Restore-from-backup added** (a whole-database or single-auction restore, with a confirm modal that re-verifies the Developer password server-side and always takes a fresh safety backup first), a **real gap fixed** where the `auctions` table itself was never included in backups/restores at all, and **logout fixed to return to SAM's own login screen** instead of navigating away to the club's external website. Run via `/ETCCSAMAll` (checkpoint then this doc update) — `test.html` was manually confirmed green by the user before the checkpoint proceeded. Everything below is now committed and the deployed code matches the commit. Also note: the **v6.11 checkpoint** (server backups switched from raw `gzcompress` bytes to real `.zip` archives) landed **earlier today, between two of today's doc updates, but its own PROJECT_STATUS.md write-up was skipped at the time** — summarized retroactively further below.
...
- **Deployed version:** **v6.12** (`index.html` footer `#app-version`) — deployed code matches the latest checkpoint commit, no drift. (Earlier today this doc twice flagged deployed-but-uncommitted work — both gaps are now closed.)
- **Git:** `main` branch, last commit `dc5d5f8` ("Checkpoint v6.12: restore-from-backup feature, auctions-table backup fix, logout fix"), pushed to `origin`. Working tree is clean.
- **`test.html` was updated this session and manually confirmed green by the user** (via https://etccapps.com/apps/sam/test.html — automated headless running is Cloudflare-blocked on this host, per the standing checkpoint procedure) before the v6.12 checkpoint proceeded.
