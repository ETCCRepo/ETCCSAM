...
**Last updated:** 2026-09-14 (later same day, third update today) — **Restore-from-backup added** (a whole-database or single-auction restore, with a confirm modal that re-verifies the Developer password server-side and always takes a fresh safety backup first), a **real gap fixed** where the `auctions` table itself was never included in backups/restores at all, and **logout fixed to return to SAM's own login screen** instead of navigating away to the club's external website. **None of this is committed yet** — see "Current state" below. Also note: the **v6.11 checkpoint** (server backups switched from raw `gzcompress` bytes to real `.zip` archives) landed and was committed/deployed **earlier today, between the last two doc updates, but its own PROJECT_STATUS.md write-up was skipped** — summarized retroactively below.
...
- **⚠️ DEPLOYED CODE IS AGAIN AHEAD OF THE LAST COMMIT.** `api.php`, `index.html`, `security-helpers.php`, and `test.html` are all modified on disk relative to the last commit (`b8468ab`, the v6.11 checkpoint) — `git status` shows all four as modified, nothing staged. This is the same pattern flagged earlier today before the v6.10 checkpoint closed it out: a run of direct bug-fix/feature requests (not a `/ETCCSAMCheckpoint`) accumulated real, deployed, tested changes with no commit yet. **Do not assume commit `b8468ab` reflects what's actually running at https://etccapps.com/apps/sam/** — it doesn't, as of this doc.
- **Deployed version still reads v6.11** (`index.html` footer `#app-version`) — no version bump for this newest round of work (none was requested; this wasn't a checkpoint).
- **Git:** `main` branch, last commit `b8468ab` ("Checkpoint v6.11: server backups now real .zip archives instead of gzcompress bytes"), pushed to `origin`. Working tree has 4 modified files (see above), uncommitted.
- **`test.html` WAS updated for this newest round of work** (unlike the previous "drift" episode earlier today) — new suites cover the logout fix, the `auctions`-table backup gap, the whole restore-from-backup feature (server + modal UI), and `restore_backup`'s server-side password re-verification. Confirmation of a green run has **not yet been explicitly obtained from the user** for this specific batch — say so plainly if asked to checkpoint, rather than assuming it's already been confirmed.
- **New in this uncommitted round of work:**
...
## What was accomplished this session (2026-09-14, later same day again — uncommitted)

Continuing directly from the v6.10 checkpoint below (same day). Three pieces of work: one bug report (logout), one real gap found while building a requested feature (backups missing the `auctions` table), and the feature itself (restore-from-backup). **None of this is committed** — see "Current state" above.
...
### 4. `test.html` updated for all three pieces above
New suites: `'Logout returns to SAM's own login screen instead of leaving the app (2026-09-14)'` (6 assertions), `'security-helpers.php — auctions table added to backup/restore scope (2026-09-14)'` (2 assertions), `'Backups — Restore from a specific backup, whole-DB or one auction (2026-09-14)'` (13 assertions covering the server side end-to-end, including the FK/transaction/safety-backup guarantees), `'Restore-from-backup UI rebuilt as a modal (2026-09-14)'` (several assertions on the modal's rendering/state machine), and `'restore_backup re-verifies the Developer password server-side, not just via the UI gate (2026-09-14)'` (4 assertions on the second-factor check). **Not yet explicitly confirmed green by the user for this batch** — say so if asked to checkpoint.

### Not done this session
- **No commit, no version bump, no checkpoint.** Deployed code (all 4 modified files) is ahead of git, same pattern as earlier today.
- **A green `test.html` run has not been explicitly confirmed by the user** for this specific batch of changes (distinct from the v6.10 batch below, which was confirmed).
- **Open items #16 (plaintext Developer password via `get_all`)** remains unaddressed, unrelated to this round of work.

### Files touched this session (uncommitted, on top of the v6.11 commit)
| File | Status | Notes |
|---|---|---|
| `index.html` | **modified, deployed, not committed** | `logoutApp()` rewritten (no more external redirect; navigates home first, relocks both auth layers, revokes maintenance bypass, closes mobile nav); new `#restore-backup-modal-host` + `renderRestoreModal()`/`restoreBackupEntry()`/`closeRestoreModal()`/`setRestoreScope()`/`performRestoreModal()`; `renderBackupLog()` extended with per-row Restore buttons and restore-aware Details/trigger-label rendering. |
| `security-helpers.php` | **modified, deployed, not committed** | New `SAM_BACKUP_TABLES`/`SAM_AUCTION_SCOPED_TABLES` constants; new `sam_read_backup_file()`, `sam_backup_auction_ids()`, `restoreDatabaseBackup()` functions; `createDatabaseBackup()`'s inline table list replaced by `SAM_BACKUP_TABLES`. |
| `api.php` | **modified, deployed, not committed** | New actions `get_backup_auctions`, `restore_backup` (added to `$allowedActions`/`$csrfProtectedActions`); new shared `sam_get_settings_password($pdo)` helper (also refactored into `verify_settings_password`). |
| `test.html` | **modified, deployed, not committed** | 5 new suites covering all of the above; not yet confirmed green by the user. |
